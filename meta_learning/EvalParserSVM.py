...
def get_optimal_hyperparameters_svm(closest_dids, n=1, p_type='automl', verbose=True):
    ...
    for dataset_id in closest_dids:
        df_res_did = get_parameters_by_did_svm(dataset_id, n=n, p_type=p_type)
...
def get_parameters_by_did_svm(did, n=1, p_type='automl'):
    ...
    for tid in tids:
        ...
        df_res_tid = get_params_svm(did, tid, n, p_type)
...
def get_params_svm(did, tid, num_results, p_type='automl'):
    ...
    df_randomforest = df[
        df['flow_name'].str.contains('^sklearn.svm.classes.SVC',regex=True,case=False) |
        df['flow_name'].str.contains('^sklearn.svm._classes.SVC',regex=True,case=False)
    ...
    if num_rows != 0:
        for metric,value,param_dict in df_randomforest[["function", "value", "parameters"]].values:
            ...
            if p_type == 'automl':
                parsed = parse_svm(param_dict)
            elif p_type == 'all':
                parsed = parse_svm_all(param_dict)
...
def parse_svm(param_dict):
    hp_my = ['kernel', 'gamma', 'C', 'degree']
    all_hp_dict = {}
    for hp in hp_my:
...
def parse_svm_all(param_dict):
    # Attention
    # The parameters are inconsistent across all OpenML evals.
    # You need to handle missing keys
    hp_default = ['C', 'kernel', 'degree', 'gamma', 'coef0', 'shrinking', 'probability', 'tol', 'class_weight', 'max_iter', 'decision_function_shape', 'break_ties']
    all_hp_dict = {}
