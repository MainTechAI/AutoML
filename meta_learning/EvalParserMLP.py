...
def get_optimal_hyperparameters_mlp(closest_dids, n=1, p_type='automl', verbose=True):
    ...
    for dataset_id in closest_dids:
        df_res_did = get_parameters_by_did_mlp(dataset_id, n=n, p_type=p_type)
...
def get_parameters_by_did_mlp(did, n=1, p_type='automl'):
    ...
    for tid in tids:
        ...
        df_res_tid = get_params_mlp(did, tid, n, p_type)
...
def get_params_mlp(did, tid, num_results, p_type='automl'):
    ...
    df_randomforest = df[
        df['flow_name'].str.contains('^sklearn.neural_network.multilayer_perceptron.MLPClassifier',regex=True,case=False) |
        df['flow_name'].str.contains('^sklearn.neural_network._multilayer_perceptron.MLPClassifier',regex=True,case=False)
    ...
    if num_rows != 0:
        for metric,value,param_dict in df_randomforest[["function", "value", "parameters"]].values:
            ...
            if p_type == 'automl':
                parsed = parse_mlp(param_dict)
            elif p_type == 'all':
                parsed = parse_mlp_all(param_dict)
...
def parse_mlp(param_dict):
    hp_my = ['hidden_layer_sizes', 'activation', 'solver', 'learning_rate', 'learning_rate_init', 'max_iter']
    all_hp_dict = {}
    for hp in hp_my:
...
def parse_mlp_all(param_dict):
    # Attention
    # The parameters are inconsistent across all OpenML evals.
    # You need to handle missing keys
    hp_default = ["hidden_layer_sizes", "activation","solver", "alpha","batch_size","learning_rate",
                  "learning_rate_init", "power_t", "max_iter","shuffle", "tol","momentum",
...
if __name__ == "__main__":
    ...
    metafeatures = X_test[3]

    n = 10
    p_type = 'automl'  # options 'automl, 'all'
    closest_dids = get_nearest_dids(metafeatures)

    df_hps = get_optimal_hyperparameters_mlp(closest_dids, n, p_type)
