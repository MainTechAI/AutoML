...
def get_optimal_hyperparameters_adaboost(closest_dids, n=1, p_type='automl', verbose=True):
    ...
    for dataset_id in closest_dids:
        df_res_did = get_parameters_by_did_adaboost(dataset_id, n=n, p_type=p_type)
...
def get_parameters_by_did_adaboost(did, n=1, p_type='automl'):
    ...
    for tid in tids:
        ...
        df_res_tid = get_params_adaboost(did, tid, n, p_type)
...
def get_params_adaboost(did, tid, num_results, p_type='automl', pipeline=True):
    ...
    df_randomforest = df[
        df['flow_name'].str.contains('^sklearn.ensemble.weight_boosting.AdaBoostClassifier',regex=True,case=False) |
        df['flow_name'].str.contains('^sklearn.ensemble._weight_boosting.AdaBoostClassifier',regex=True,case=False)
    ...
    if num_rows != 0:
        for metric,value,param_dict in df_randomforest[["function", "value", "parameters"]].values:
            ...
            if p_type == 'automl':
                parsed = parse_adaboost(param_dict)
            elif p_type == 'all':
                parsed = parse_adaboost_all(param_dict)
...
def parse_adaboost(param_dict):
    hp_my = ['base_estimator','learning_rate']
    all_hp_dict = {}
    for hp in hp_my:
...
if __name__ == "__main__":
    ...
    df_hps = get_optimal_hyperparameters_adaboost(closest_dids, n, p_type)
